...
DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_LEFT_CONTROL, DP_TYPE_ENUM},
  {DPID_LEFT_PERCENT, DP_TYPE_VALUE},
  {DPID_LEFT_CALIBRATION, DP_TYPE_BOOL},
  {DPID_RIGHT_CONTROL, DP_TYPE_ENUM},
  {DPID_RIGHT_PERCENT, DP_TYPE_VALUE},
  {DPID_RIGHT_CALIBRATION, DP_TYPE_BOOL},
  {DPID_CURTAIN_CONTROL, DP_TYPE_ENUM},
...
void all_data_update(void)
{
    ...
    mcu_dp_enum_update(DPID_LEFT_CONTROL,当前左窗户); //枚举型数据上报;
    mcu_dp_value_update(DPID_LEFT_PERCENT,当前左窗户); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_LEFT_CALIBRATION,当前左窗户校准); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_RIGHT_CONTROL,当前右窗户); //枚举型数据上报;
    mcu_dp_value_update(DPID_RIGHT_PERCENT,当前右窗户); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_RIGHT_CALIBRATION,当前右窗户校准); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_CURTAIN_CONTROL,当前窗帘控制); //枚举型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_left_control_handle
功能描述 : 针对DPID_LEFT_CONTROL的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_left_control_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char left_control;
    
    left_control = mcu_get_dp_download_enum(value,length);
    switch(left_control) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_LEFT_CONTROL, left_control);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_left_percent_handle
功能描述 : 针对DPID_LEFT_PERCENT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_left_percent_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long left_percent;
    
    left_percent = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_LEFT_PERCENT,left_percent);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_left_calibration_handle
功能描述 : 针对DPID_LEFT_CALIBRATION的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_left_calibration_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char left_calibration;
    
    left_calibration = mcu_get_dp_download_bool(value,length);
    if(left_calibration == 0) {
        //开关关
    }else {
        //开关开
    }
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_LEFT_CALIBRATION,left_calibration);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_right_control_handle
功能描述 : 针对DPID_RIGHT_CONTROL的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_right_control_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char right_control;
    
    right_control = mcu_get_dp_download_enum(value,length);
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_RIGHT_CONTROL, right_control);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_right_percent_handle
功能描述 : 针对DPID_RIGHT_PERCENT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_right_percent_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long right_percent;
    
    right_percent = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_RIGHT_PERCENT,right_percent);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_right_calibration_handle
功能描述 : 针对DPID_RIGHT_CALIBRATION的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_right_calibration_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char right_calibration;
    
    right_calibration = mcu_get_dp_download_bool(value,length);
    if(right_calibration == 0) {
        //开关关
    }else {
        //开关开
    }
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_RIGHT_CALIBRATION,right_calibration);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_curtain_control_handle
功能描述 : 针对DPID_CURTAIN_CONTROL的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_curtain_control_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char curtain_control;
    
    curtain_control = mcu_get_dp_download_enum(value,length);
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_CURTAIN_CONTROL, curtain_control);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_curtain_percent_handle
功能描述 : 针对DPID_CURTAIN_PERCENT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_curtain_percent_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long curtain_percent;
    
    curtain_percent = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_CURTAIN_PERCENT,curtain_percent);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_curtain_calibration_handle
功能描述 : 针对DPID_CURTAIN_CALIBRATION的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_curtain_calibration_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char curtain_calibration;
    
    curtain_calibration = mcu_get_dp_download_bool(value,length);
    if(curtain_calibration == 0) {
        //开关关
    }else {
        //开关开
    }
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_CURTAIN_CALIBRATION,curtain_calibration);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_control_mode_handle
功能描述 : 针对DPID_CONTROL_MODE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
static unsigned char dp_download_control_mode_handle(const unsigned char value[], unsigned short length)
{
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_CONTROL_MODE,control_mode);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
        case DPID_LEFT_CONTROL:
            //左窗户控制处理函数
            ret = dp_download_left_control_handle(value,length);
        break;
        case DPID_LEFT_PERCENT:
            //左窗户百分比处理函数
            ret = dp_download_left_percent_handle(value,length);
        break;
        case DPID_LEFT_CALIBRATION:
            //左窗户校准处理函数
            ret = dp_download_left_calibration_handle(value,length);
        break;
        case DPID_RIGHT_CONTROL:
            //右窗户控制处理函数
            ret = dp_download_right_control_handle(value,length);
        break;
        case DPID_RIGHT_PERCENT:
            //右窗户百分比处理函数
            ret = dp_download_right_percent_handle(value,length);
        break;
        case DPID_RIGHT_CALIBRATION:
            //右窗户校准处理函数
            ret = dp_download_right_calibration_handle(value,length);
        break;
        case DPID_CURTAIN_CONTROL:
            //窗帘控制处理函数
            ret = dp_download_curtain_control_handle(value,length);
        break;
        case DPID_CURTAIN_PERCENT:
            //窗帘百分比处理函数
            ret = dp_download_curtain_percent_handle(value,length);
        break;
        case DPID_CURTAIN_CALIBRATION:
            //窗帘校准处理函数
            ret = dp_download_curtain_calibration_handle(value,length);
        break;
        case DPID_CONTROL_MODE:
            //自动模式处理函数
            ret = dp_download_control_mode_handle(value,length);
        break;
...
void mcu_get_greentime(unsigned char time[])
{
    ...
    */
    if(time[0] == 1) {
        //正确接收到wifi模块返回的格林数据,时间处理在这time[4]
...
void weather_data_user_handle(char *name, unsigned char type, const unsigned char *data, char day)
{
    ...
    //注意要根据所选参数类型来获得参数值！！！这里获取天气参数存入全局变量
    if(my_strcmp(name, "pm25") == 0) {
        if (value_int >=115) {    //PM25>=115则不适合开窗
          pm25_sta = 1;
        }else{
          pm25_sta = 0;
        }
